...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **SRP** 单一职责 | ✅ 遵循 | 将对象的**创建逻辑**（工厂类）与**业务逻辑**（产品类）分离，各自职责单一 |
| **DIP** 依赖倒转 | ✅ 遵循 | 客户端依赖抽象产品类 `AbstractPay`，而非具体产品类 |
| **OCP** 开闭原则 | ❌ 违反 | 增加新产品时必须修改工厂类的判断逻辑（`if-else`），是课件明确指出的主要缺点 |
| **LSP** 里氏代换 | ✅ 遵循 | 具体产品类可以替换抽象产品类使用 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **OCP** 开闭原则 | ✅ 遵循 | **核心优势**：增加新产品只需新增具体工厂类和具体产品类，无须修改已有代码 |
| **DIP** 依赖倒转 | ✅ 遵循 | 客户端依赖抽象工厂 `PayMethodFactory` 和抽象产品 `AbstractPay`，不依赖具体类 |
| **SRP** 单一职责 | ✅ 遵循 | 每个具体工厂只负责创建一种具体产品，职责单一 |
| **LSP** 里氏代换 | ✅ 遵循 | 具体工厂和具体产品均可替换各自的抽象父类 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **OCP** 开闭原则 | ⚠️ 倾斜 | **增加产品族**容易（只需新增具体工厂），但**增加产品等级结构**困难（需修改抽象工厂及所有子类）——课件称为"开闭原则的倾斜性" |
| **DIP** 依赖倒转 | ✅ 遵循 | 客户端依赖抽象工厂和抽象产品接口，不依赖具体实现 |
| **SRP** 单一职责 | ✅ 遵循 | 每个具体工厂负责创建属于同一产品族的一组相关对象 |
| **ISP** 接口隔离 | ✅ 遵循 | 抽象工厂为不同产品族定义独立接口，客户端只使用需要的产品族 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **SRP** 单一职责 | ❌ 违反 | 单例类承担了**双重职责**：既要管理自身业务逻辑，又要控制自身的创建和生命周期，是课件中老手与新手对话隐含讨论的问题 |
| **OCP** 开闭原则 | ❌ 违反 | 单例类难以扩展，对其子类化或修改通常需要改动单例类本身的代码 |
| **DIP** 依赖倒转 | ❌ 违反 | 客户端直接依赖具体单例类（通过 `getInstance()` 静态方法），而非抽象接口 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **OCP** 开闭原则 | ✅ 遵循 | 增加新的具体建造者无须修改指挥者和已有建造者代码 |
| **SRP** 单一职责 | ✅ 遵循 | Builder 负责部件构建，Director 负责构建顺序，Product 负责自身表示——三者职责分离 |
| **DIP** 依赖倒转 | ✅ 遵循 | Director 针对抽象 Builder 编程，不依赖具体建造者 |
| **LSP** 里氏代换 | ✅ 遵循 | 具体建造者可以替换抽象建造者使用 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **OCP** 开闭原则 | ❌ 违反 | 为已有类增加克隆方法必须**修改其源代码**，课件明确指出这一点 |
| **SRP** 单一职责 | ⚠️ 可能违反 | 原型类在承担自身业务逻辑的同时，还需要实现 `clone()` 方法来支持自我复制 |
| **LSP** 里氏代换 | ✅ 遵循 | 克隆对象与原对象类型相同，可以互相替换 |
...
策略模式是课件中**第一个引入的设计模式**，其动机本身就是三条核心设计原则的直接体现：

| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **OCP** 开闭原则 | ✅ 遵循 | **封装变化**：将易变的行为（fly/quack）封装为独立策略类，新增行为无需修改 Duck 或已有策略 |
| **DIP** 依赖倒转 | ✅ 遵循 | **针对接口编程**：Duck 依赖 `FlyBehavior`/`QuackBehavior` 接口，而非具体飞行/叫声实现 |
| **CRP** 合成复用 | ✅ 遵循 | **多用组合，少用继承**：Duck 通过组合持有行为对象，而非通过继承获得行为。课件用 SimUDuck 案例从继承→接口→组合的演进完整展示了这一原则 |
| **SRP** 单一职责 | ✅ 遵循 | 每个具体策略类只封装一种算法，职责单一 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **SRP** 单一职责 | ✅ 遵循 | 每个具体状态类只封装**一种状态下的行为**，避免了环境类中庞大的 `if-else` 条件分支 |
| **OCP** 开闭原则 | ⚠️ 视情况 | **简单状态模式**（状态间无切换）：增加新状态无需修改已有代码，符合 OCP。**可切换状态模式**：增加新状态需修改其他状态类的转换代码，违反 OCP |
| **LSP** 里氏代换 | ✅ 遵循 | 具体状态类可替换抽象状态类，环境类行为随之改变 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **SRP** 单一职责 | ✅ 遵循 | 将**发出命令的责任**（Invoker）与**执行命令的责任**（Receiver）分离，Command 对象封装请求 |
| **OCP** 开闭原则 | ✅ 遵循 | 增加新命令只需新增具体命令类，无须修改 Invoker 和已有命令 |
| **DIP** 依赖倒转 | ✅ 遵循 | Invoker 依赖抽象 `Command` 接口，不依赖具体命令或接收者 |
| **LSP** 里氏代换 | ✅ 遵循 | 具体命令可替换抽象命令，Invoker 无须感知具体命令类型 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **OCP** 开闭原则 | ✅ 遵循 | 增加新的观察者无须修改目标（Subject）代码，课件明确指出 |
| **DIP** 依赖倒转 | ✅ 遵循 | Subject 依赖 `Observer` 抽象接口，不依赖具体观察者类 |
| **LoD** 迪米特法则 | ✅ 遵循 | Subject 只知道 Observer 接口的存在，不知道具体观察者的内部细节——目标与观察者之间是**抽象耦合** |
| **SRP** 单一职责 | ✅ 遵循 | Subject 负责状态管理，Observer 负责响应变化，职责分离 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **LoD** 迪米特法则 | ✅ 遵循 | **课件明确指出**中介者模式是迪米特法则的**典型应用**。同事类只与中介者通信，不直接引用其他同事，将网状结构变为星形结构 |
| **SRP** 单一职责 | ✅ 遵循 | 交互协调逻辑集中在中介者中，同事类只需关注自身业务 |
| **OCP** 开闭原则 | ⚠️ 部分遵循 | 增加新同事类相对容易；但如果交互逻辑变化，需要修改具体中介者类 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **OCP** 开闭原则 | ✅ 遵循 | 算法的**不变部分**在父类中固化（对修改封闭），**可变部分**通过子类覆盖实现（对扩展开放） |
| **SRP** 单一职责 | ✅ 遵循 | 父类负责算法骨架，子类负责具体步骤实现——各司其职，类的内聚性提高 |
| **好莱坞原则** | ✅ 遵循 | *"Don't call us, we'll call you."* ——父类控制调用时机，子类只提供实现，不主动调用父类 |
| **LSP** 里氏代换 | ✅ 遵循 | 子类可以替换父类使用，且不改变算法的整体结构 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **SRP** 单一职责 | ✅ 遵循 | **课件明确指出**：将**遍历任务**放在迭代器对象上而非聚合对象上，使得聚合类只需关注集合管理，迭代器只关注遍历逻辑——各自只有一个变化原因 |
| **OCP** 开闭原则 | ✅ 遵循 | 增加新的遍历方式只需新增迭代器类，无须修改聚合类 |
| **DIP** 依赖倒转 | ✅ 遵循 | 客户端通过 `Iterator` 接口遍历，不依赖具体聚合类的内部实现（如 Array 还是 ArrayList） |

> **课件重点**：迭代器模式是**单一职责原则（SRP）** 的直接引入点——每个职责都是潜在的变化轴，多于一个职责意味着多于一个变化领域。
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **OCP** 开闭原则 | ✅ 遵循 | 在不修改已有类（Adaptee）和客户端代码的情况下，通过引入适配器类重用现有功能，课件明确指出 |
| **SRP** 单一职责 | ✅ 遵循 | 适配器类专注于接口转换这一单一职责，Adaptee 保持原有职责不变 |
| **DIP** 依赖倒转 | ✅ 遵循 | 客户端针对 Target 抽象接口编程，不直接依赖 Adaptee |
| **CRP** 合成复用 | ✅ 遵循 | 对象适配器通过**组合**持有 Adaptee 实例，而非继承（类适配器则使用继承） |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **LSP** 里氏代换 | ✅ 遵循 | 叶子对象和容器对象可以**一致对待**，客户端无须区分，这正是组合模式的核心目标 |
| **DIP** 依赖倒转 | ✅ 遵循 | 客户端针对抽象构件 `Component` 编程，不依赖具体的 Leaf 或 Composite |
| **SRP** 单一职责 | ❌ 透明模式下违反 | **透明组合模式**中，`Component` 既定义叶子操作又定义容器管理方法（`add`/`remove`/`getChild`），叶子类被迫实现不需要的方法。**安全组合模式**可解决此问题，但牺牲了透明性 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **SRP** 单一职责 | ✅ 遵循 | 将**抽象维度**与**实现维度**分离为两个独立的类层次，各自只有一个变化原因 |
| **OCP** 开闭原则 | ✅ 遵循 | 两个维度可以**独立扩展**，任意扩展一个维度都不需要修改另一个维度的代码 |
| **CRP** 合成复用 | ✅ 遵循 | **核心动机**：用关联（组合）替代多层继承。课件以"形状×颜色"为例说明：继承方案需要 N×M 个子类，桥接只需 N+M 个类 |
| **DIP** 依赖倒转 | ✅ 遵循 | Abstraction 依赖 Implementor 接口，而非具体实现类 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **OCP** 开闭原则 | ✅ 遵循 | **核心优势**：在不修改已有类的情况下动态扩展功能。具体构件类和具体装饰类可以独立变化，新增装饰类无需修改已有代码 |
| **CRP** 合成复用 | ✅ 遵循 | **核心动机**：用组合替代继承来扩展功能。继承是静态的、编译时确定的；装饰组合是动态的、运行时灵活的 |
| **SRP** 单一职责 | ✅ 遵循 | 每个具体装饰类只添加**一种**额外职责，构件类专注于核心功能 |
| **LSP** 里氏代换 | ✅ 遵循 | 装饰后的对象与原始构件类型相同（同属 Component），可以互相替换 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **LoD** 迪米特法则 | ✅ 遵循 | **课件明确指出**是迪米特法则的体现：外观类将客户端所涉及的子系统协作伙伴数量减到最少，充当客户与子系统之间的"第三者" |
| **SRP** 单一职责 | ✅ 遵循 | 外观类提供统一入口，将子系统复杂性封装在内部；每个子系统类也保持各自的单一职责 |
| **OCP** 开闭原则 | ❌ 违反 | 课件明确指出：无抽象外观类时，增加新子系统需要修改外观类源代码。可通过**引入抽象外观类**解决 |
| **ISP** 接口隔离 | ✅ 遵循 | 外观模式为客户端提供一个简化的高层接口，隐藏了客户端不需要关心的子系统细节 |
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **SRP** 单一职责 | ⚠️ 复杂化 | 享元模式分离**内部状态**（可共享）与**外部状态**（不可共享），这使得享元对象职责更纯粹；但整体系统逻辑因状态分离而变得复杂 |
| **OCP** 开闭原则 | ✅ 遵循 | 增加新的享元类只需继承抽象享元类，享元工厂通过简单工厂模式管理对象创建 |
| **ISP** 接口隔离 | ✅ 遵循 | 抽象享元接口仅定义必要操作，外部状态通过参数传入，客户端不需要了解内部状态 |


> **说明**：享元模式的关注点是**性能优化**而非设计原则，它通过共享技术以空间换时间。与其他模式联用时（简单工厂创建享元、单例管理工厂、组合模式形成复合享元），各自的职责分离有助于遵循 SRP。
...
| 原则 | 遵循/违反 | 说明 |
|------|-----------|------|
| **SRP** 单一职责 | ✅ 遵循 | **核心体现**：代理类负责**访问控制**（权限检查、延迟加载、远程通信等），真实主题类负责**业务逻辑**，两者职责严格分离 |
| **OCP** 开闭原则 | ✅ 遵循 | 通过代理扩展功能（如添加缓存、日志、权限控制）无需修改真实主题类的代码 |
| **DIP** 依赖倒转 | ✅ 遵循 | 客户端依赖抽象 Subject 接口，代理和真实主题均实现同一接口，客户端不感知代理的存在 |
| **LSP** 里氏代换 | ✅ 遵循 | 代理对象可以**完全替换**真实主题对象使用，客户端无需区分 |
